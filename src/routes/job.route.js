...
const { createJob, matchArtisans, acceptJob, updateJobStatus, suggestJobs,assignArtisan,getActiveJobs,getJobHistory,
    cancelJob, getAllJobs, applyForJob, getJobApplications
...
router.post("/apply/:jobId", protect, authorizeRoles("artisan"),applyForJob);
router.get("/applications/:jobId", protect, getJobApplications);
router.post('/assign/:jobId', protect, authorizeRoles("customer", "admin"), assignArtisan);
